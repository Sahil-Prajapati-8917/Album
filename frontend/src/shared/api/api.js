...
// Song functions
export const searchYouTubeSongs = async (query) => {
  return await apiRequest(`/songs/search?query=${encodeURIComponent(query)}`);
};

export const downloadYouTubeSong = async (songData) => {
  return await apiRequest('/songs/download', {
    method: 'POST',
