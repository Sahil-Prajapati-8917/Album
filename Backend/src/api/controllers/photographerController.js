const photographerRepository = require('../../data/repositories/PhotographerRepository');
const { sanitizeString } = require('../../common/middleware/validation');
...
exports.createPhotographer = async (req, res) => {
    try {
        ...
        const photographer = await photographerRepository.create({
...
exports.getMyPhotographers = async (req, res) => {
    try {
        const photographers = await photographerRepository.find(
            { userId: req.user.id },
            { sort: { name: 1 } }
        );
        res.status(200).json({
...
exports.updatePhotographer = async (req, res) => {
    try {
        ...
        const photographer = await photographerRepository.findOneAndUpdate(
...
exports.deletePhotographer = async (req, res) => {
    try {
        const photographer = await photographerRepository.findOneAndDelete({ _id: req.params.id, userId: req.user.id });
        if (!photographer) {
            return res.status(404).json({ success: false, message: 'Photographer not found' });
        }
        res.status(200).json({
